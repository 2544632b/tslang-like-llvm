...
TernaryOperatorExprNode::TernaryOperatorExprNode(
    const SharedPtr<ExprNode> &bhs,
    const SharedPtr<ExprNode> &lhs,
    const SharedPtr<ExprNode> &rhs
) : bhs(bhs),
    lhs(lhs),
...
void TernaryOperatorExprNode::accept(const SharedPtr<ASTVisitor> &visitor) {
    visitor->visit(staticPtrCast<TernaryOperatorExprNode>(shared_from_this()));
}

void TernaryOperatorExprNode::bindChildrenInversely() {
    bhs->parent = lhs->parent = rhs->parent = shared_from_this();
}
