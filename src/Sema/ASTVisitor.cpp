...
void ASTVisitor::visit(const SharedPtr<TernaryOperatorExprNode> &topExpr) {
    topExpr->bhs->accept(shared_from_this());
    topExpr->lhs->accept(shared_from_this());
    topExpr->rhs->accept(shared_from_this());
}
