...
/// 三元运算表达式节点
class TernaryOperatorExprNode : public ExprNode {
public:
    TernaryOperatorExprNode(const SharedPtr<ExprNode> &bhs, const SharedPtr<ExprNode> &lhs, const SharedPtr<ExprNode> &rhs);

    ~TernaryOperatorExprNode() override = default;

    void bindChildrenInversely() override;

    void accept(const SharedPtr<ASTVisitor> &visitor) override;

    SharedPtr<ExprNode> bhs = nullptr, lhs = nullptr, rhs = nullptr;
};
